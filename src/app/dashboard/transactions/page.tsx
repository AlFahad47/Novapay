...
export default function TransactionsPage() {
  ...
  const downloadSingleReceipt = (tx: any) => {
    ...
    doc.text(`Amount: ${Number(tx.amount).toFixed(2)} ${tx.currency}`, 10, 65);
          ...
          {transactions.length > 0 ? (
            <div className="min-w-full divide-y dark:divide-blue-900">
              {transactions.map((tx: any) => {
                ...
                if (isDonation) {
                  amountDisplay = `-${Number(tx.amount).toFixed(2)} BDT`;
                  amountColor   = "text-[#e63b60]";
                  subLabel      = `Donated to ${tx.campaignTitle}`;
                } else if (isIntlSend) {
                  amountDisplay = `-${Number(tx.amountSent).toFixed(2)} ${tx.fromCurrency}`;
                  amountColor   = "text-rose-500";
                  subLabel      = `→ ${Number(tx.amountReceived).toFixed(2)} ${tx.toCurrency} • Fee: ${Number(tx.fee).toFixed(2)} ${tx.fromCurrency}`;
                } else if (isIntlReceive) {
                  amountDisplay = `+${Number(tx.amountReceived).toFixed(2)} ${tx.toCurrency}`;
                  amountColor   = "text-blue-500";
                  subLabel      = `from ${tx.senderEmail}`;
                } else if (isTopUp) {
                  amountDisplay = `+${Number(tx.amountAdded).toFixed(2)} ${tx.toCurrency}`;
                  amountColor   = "text-purple-500";
                  subLabel      = `−${Number(tx.amountDeducted).toFixed(2)} ${tx.fromCurrency} from main balance`;
                } else {
                  amountDisplay = `${isExpense ? "-" : "+"}${Number(tx.amount).toFixed(2)}`;
