...
const BannerUser: React.FC = () => {
  ...
  const cardActions = [
    ...
    { label: "Request", icon: <TrendingUp size={15} />, onClick: () => setActiveModal("request") },
  ];
  ...
  const confirmPayment = async (data: any) => {
    Swal.fire({
      ...
      text: `Send ${currencySymbol}${data.amount} to ${data.from || data.senderEmail}?`,
      icon: "warning",
  ...
  return (
    <section className="relative w-full min-h-[88vh] bg-[#f0f7ff] dark:bg-[#050B14] flex flex-col items-center justify-center pt-24 pb-16 overflow-hidden font-sans">
      ...
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-150 h-100 dark:bg-[#1E50FF] opacity-[0.18] blur-[140px] rounded-full pointer-events-none z-[-1]" />
        ...
        >
          <div className="flex gap-2">
            ...
            <motion.button
              ...
              whileHover={{ y: -2 }}
              whileTap={{ scale: 0.98 }}
              className="flex items-center gap-3 px-4 py-2 rounded-xl bg-white/40 dark:bg-white/3 border border-white/20 dark:border-white/8 backdrop-blur-md shadow-sm hover:shadow-md hover:bg-white/60 dark:hover:bg-white/6 transition-all duration-300"
            >
              ...
              <div className="flex flex-col items-start leading-tight">
                <span className="text-[9px] font-medium uppercase tracking-[0.15em] text-[#64748B] dark:text-[#94A3B8]">Tier Status</span>
                <span className="text-sm font-semibold text-[#0F172A] dark:text-white">{dbUser?.rank || "Bronze"}</span>
              </div>
          ...
          <div>
            ...
            <p className="mt-4 text-[#64748B] dark:text-[#94A3B8] text-sm md:text-base max-w-md leading-relaxed">
              Your digital assets are {isApproved ? "secure and ready" : "under review"}.{" "}
              {isApproved
          ...
          <div className="flex flex-wrap items-center gap-4">
            <Link href="/dashboard">
              ...
              >
                <div className="absolute inset-0 bg-linear-to-r from-[#4DA1FF] to-[#1E50FF]" />
            ...
            <Link href="/wallet">
              <motion.button
                whileHover={{ scale: 1.04 }}
                className="flex items-center gap-2 px-7 py-3.5 rounded-full border border-[#4DA1FF]/25 bg-white/70 dark:bg-white/4 backdrop-blur-md text-[#1E50FF] dark:text-[#4DA1FF]"
              >
                <Wallet size={15} />
                <span className="text-sm font-semibold cursor-pointer">Wallet Account</span>
              </motion.button>
            </Link>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            {stats.map((stat, i) => (
              <motion.div
                ...
                className="flex items-center gap-2 px-3 py-2 rounded-xl border border-[#4DA1FF]/15 bg-white/60 dark:bg-white/4 backdrop-blur-sm"
              >
        ...
        >
          ...
          <motion.button
            ...
            className={`relative flex items-center gap-2 self-end px-3 py-1.5 rounded-full border backdrop-blur-md transition-all duration-500 cursor-pointer ${
              pendingRequests > 0
                ? "bg-red-50 dark:bg-red-500/10 border-red-500 shadow-lg shadow-red-500/20 active:scale-95"
                : "bg-white/70 dark:bg-[#0F172A]/70 border-[#4DA1FF]/20"
            }`}
          >
          ...
          <div className="float-card">
            <div
              className={`shimmer-card w-75 md:w-85 h-50 md:h-55 rounded-2xl border border-white/20 p-6 flex flex-col justify-between shadow-2xl transition-all duration-500 ${
                !isApproved ? "grayscale brightness-75" : ""
              }`}
          ...
          <div className="flex items-center gap-3 w-full px-2">
            {cardActions.map((action) => (
              <motion.button
                ...
                whileHover={{ y: -2 }}
                whileTap={{ scale: 0.95 }}
                className="flex-1 flex flex-col items-center gap-1.5 py-3 rounded-xl bg-white/70 dark:bg-white/4 border border-[#4DA1FF]/15 backdrop-blur-sm hover:bg-[#4DA1FF]/10 text-[#1E50FF] dark:text-[#4DA1FF]"
              >
              ...
              <div className="flex flex-col items-center text-center">
                ...
                <div className="flex gap-3 w-full">
                  ...
                  <button
                    ...
                    className="flex-1 py-4 rounded-2xl bg-linear-to-r from-[#4DA1FF] to-[#1E50FF] text-white text-sm font-bold shadow-lg shadow-[#1E50FF]/30 hover:shadow-[#1E50FF]/50 transition-all"
                  >
                    Pay Now
                  </button>
                </div>
