    ...
    >
      ...
      >
        ...
        <div className="flex flex-col items-center text-center">
          <div className={`w-20 h-20 rounded-3xl flex items-center justify-center mb-6 rotate-12 shadow-xl ${
            isApproved ? 'bg-green-500 shadow-green-500/20' : 
            isPending ? 'bg-orange-500 shadow-orange-500/20' : 'bg-[#1E50FF] shadow-blue-500/20'
          }`}>
            <div className="-rotate-12 text-white">
              {isApproved ? <ShieldCheck size={40} /> : isPending ? <Timer size={40} /> : <ShieldAlert size={40} />}
            </div>
          </div>

          <h2 className="text-2xl font-bold text-[#0F172A] dark:text-white mb-2">
            {isApproved ? "Account Verified!" : isPending ? "Review in Progress" : "Complete Your Profile"}
          </h2>
          ...
          <div className="w-full space-y-3">
            <h4 className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#4DA1FF] mb-4 text-left border-b border-[#4DA1FF]/10 pb-2">Next Steps & Terms</h4>
            <ul className="text-left space-y-3 mb-8">
              {[
                isApproved ? "Access all dashboard widgets" : "Submit valid National ID/Passport",
                "Verify Your Identity for Limit Increase",
                "Do not share your PIN with anyone"
              ].map((term, i) => (
                <li key={i} className="flex items-start gap-3 text-xs text-[#64748B] dark:text-[#94A3B8]">
            ...
            <button 
              onClick={onClose}
              className="w-full py-4 rounded-2xl bg-gradient-to-r from-[#4DA1FF] to-[#1E50FF] text-white font-bold shadow-lg shadow-[#1E50FF]/30 hover:scale-[1.02] transition-transform"
            >
              {isApproved ? "Get Started" : isPending ? "I Understand" : "Verify Now"}
            </button></Link>
          </div>
        </div>
      </motion.div>
